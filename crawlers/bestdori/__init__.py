...
class CardCrawler(Crawler):
    def __init__(self, logger, table, savedir_config, json_api):
        ...
        if not os.path.exists(os.path.join(self.savedir_config['assets'], self.thumb_savedirname)):
            os.makedirs(os.path.join(self.savedir_config['assets'], self.thumb_savedirname))
    ...
    def init(self):
        ...
        all_skills = self.request_overall_json('all_skills')
        self.logger.info(f'get all_skills.json: {len(all_skills)}')
        all_data = self.request_overall_json('all_cards')
        l = len(all_data)
        finished = 0
        for id_, data in all_data.items():
            self.logger.info(f'requesting card data {finished}/{l}')
            overall = {}
            if data['rarity'] < 3:
    ...
    def update(self):
        ...
        diff = all_cards - local_all_cards
        l = len(diff)
        finished = 0
        for id_ in diff:
            self.logger.info(f'requesting card data {finished}/{l}')
...
class EventCrawler(Crawler):
    def __init__(self, logger, table, savedir_config, json_api):
        ...
        for server in self.servers:
            if not os.path.exists(os.path.join(self.savedir_config['assets'], server)):
                os.makedirs(os.path.join(self.savedir_config['assets'], server))
    ...
    def request_assets(self, id_):
        try:
            ...
            if not os.path.exists(os.path.join(self.savedir_config['assets'], asset_bundle_name)):
                os.makedirs(os.path.join(self.savedir_config['assets'], asset_bundle_name))
    ...
    def init(self):
        ...
        finished = 0
        for id_, data in all_data.items():
            self.logger.info(f'requesting event data {finished}/{l}')
            if self.request_data(int(id_)) == 0:
                time.sleep(random.uniform(1, 2))
    ...
    def update(self):
        ...
        diff = all_events - local_all_events
        l = len(diff)
        finished = 0
        for id_ in diff:
            self.logger.info(f'requesting event data {finished}/{l}')
        ...
        for id_ in local_all_events:
            self.logger.info(f'requesting event data {finished}/{len(local_all_events)}')
...
class GachaCrawler(Crawler):
    ...
    def init(self):
        ...
        finished = 0
        for id_, data in all_data.items():
            self.logger.info(f'requesting gacha data {finished}/{l}')
            if self.request_data(int(id_)) == 0:
                time.sleep(random.uniform(1, 2))
    ...
    def update(self):
        ...
        diff = all_gachas - local_all_gachas
        l = len(diff)
        finished = 0
        for id_ in diff:
            self.logger.info(f'requesting gacha data {finished}/{l}')
        ...
        for id_ in local_all_gachas:
            self.logger.info(f'updating gacha data {finished}/{len(local_all_gachas)}')
            local_data = self.load_json(id_)
            for i, server in enumerate(self.servers[1:]):
        ...
        try:
            ...
            for i, server in enumerate(self.servers):
                if json_data['publishedAt'][i]:
                    ...
                    if not os.path.exists(os.path.join(self.savedir_config['assets'], resourceName, server)):
                        os.makedirs(os.path.join(self.savedir_config['assets'], resourceName, server))
