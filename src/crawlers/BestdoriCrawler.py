...
from . import Logger
from bestdori import CardTable, CardCrawler, EventTable, EventCrawler, GachaTable, GachaCrawler
...
parser.add_argument('-d', '--datapath', type=str, help='static resource path', default=os.path.abspath('./data/image/assets/'))

if __name__ == '__main__':
    args = parser.parse_args()
    work_dir = os.path.dirname(os.path.abspath(__file__))
    log_file = os.path.join(work_dir, 'crawler.log')
    cache_dir = os.path.join(os.path.dirname(work_dir), 'cache')
    asset_dir = args.datapath
    json_dir = os.path.join(cache_dir, 'json')
    db_file = os.path.join(cache_dir, 'bestdori.db')
