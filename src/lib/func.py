...
if os.access(os.path.join(const.user_profile_path, 'cur_back_pic.json'), os.R_OK):
    with open(os.path.join(const.user_profile_path, 'cur_back_pic.json'), 'r', encoding='utf-8') as f:
        cur_back_pic = json.load(f)
else:
    cur_back_pic = {}
bilibili_drawcard_spider = Bilibili_DrawCard_Spider()

async def query_card(send_handler, msg, receiver_id):   # send_handler: Bot send handler
...
async def query_event(send_handler, msg, receiver_id):
...
async def query_gacha(send_handler, msg, receiver_id):
...
async def query_user_gacha(send_handler, msg, receiver_id, logger=None):
    if msg == '更新抽卡数据':
        ...
        logger.info(f'{msg}, {res}')
        ret = []
        query = False
        if res:
            logger.info(f'query gacha user {res.group(1)}')
            query = True
            ...
            if res:
                uid = int(res.group(1))
                logger.info(f'query gacha user {uid}')
                query = True
        ...
        texts = []
        stringmsg = []
        logger.info(f'gacha result {ret}')
        too_large = False
        if len(ret) > 50:
            ret = ret[:50]
            too_large = True
...
def _query_room_number(logger=None):
    ...
    if r.status_code == 200:
        ...
        try:
            ...
            logger.error('bad reply of bandoristation: %s', r)
        ret.reverse()
        return ret
    else:
        return None

async def fixed_roomcode_reply(send_handler, msg, receiver_id, sender_id, logger=None):
    if msg == 'ycm' or msg == '有车吗':
        rl = _query_room_number()
        if rl is None:
        ...
        if res:
            ...
            flag, msg = _submit_room_number(room_code, sender_id, msg)
            if flag:
...
async def fixed_reply(send_handler, msg, receiver_id, logger=None):
    if msg == '使用说明':
...
async def change_back_jpg(send_handler, msg, receiver_id, sender_id, user_profile, logger=None):
    ...
    if back_num and back_num in ImageProcesser.CUR_BACK_PIC_SET:
        logger.info(f'{sender_id} change back {back_num} success')
        user_profile[sender_id] = int(back_num)
        await send_handler(receiver_id, StringMsg('已修改'))
        with open(os.path.join(const.user_profile_path, 'cur_back_pic.json'), 'w', encoding='utf-8') as f:
            json.dump(user_profile, f, ensure_ascii=False)
        return True

async def handle_jpg(send_handler, msg, receiver_id, sender_id, user_profile, logger=None):
    sender_id = str(sender_id)
    if not msg.endswith('.jpg') or '[' in msg:
        return False
    msg = msg[:-4]
    file_name = ImageProcesser.image_merge(user_profile.get(sender_id, 31), msg)
    await send_handler(receiver_id, ImageMsg({'file': file_name}))
    return True