...
parser.add_argument('-c', '--content', type=str,
                    help='card, event or gacha', nargs='*')
parser.add_argument('-d', '--datapath', type=str, help='static resource path',
                    default=os.path.abspath('./cq/data/image/assets/'))
...
if __name__ == '__main__':
    ...
    db_file = os.path.join(data_dir, 'bestdori.db')
    ...
    if args.operation in ['init', 'update']:
        content = set(args.content or ['card', 'event', 'gacha']) & set(
            ['card', 'event', 'gacha'])
        for c in content:
